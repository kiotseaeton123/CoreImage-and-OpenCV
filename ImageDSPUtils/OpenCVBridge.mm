...
-(bool)processFinger{ //2.1
    ...
    //if (green and blue are less than 30) OR (red is greater than 200 and blue is less than 30)
    
    //display the (numBeats)/(frameCount)
    
    //sprintf(beatText, "Beat %.0f , frameCount %.0f", numBeats/4, frameCount);
    ...
    if ((avgPixelIntensity.val[2]<40 && avgPixelIntensity.val[1]<40) || (avgPixelIntensity.val[0]>200 && avgPixelIntensity.val[2]<50)) { //then read data into buffer, then return true
        ...
        if (self.arrayID < numFrames){ //when array full
            ...
            //detect heart rate: detect fluctuation: peak finding from array
//            if (peakFinding){
//                char beatText[50];
//                sprintf(beatText, "beat");
//                cv::putText(_image, beatText, cv::Point(0,50), FONT_HERSHEY_PLAIN, 0.75, Scalar::all(255), 1, 2);
//            }
//            if (self.arrayID >9){
//                id first = self.redArr[self.arrayID-2];
//                id second = self.redArr[self.arrayID-1];
//                id third = self.redArr[self.arrayID];
