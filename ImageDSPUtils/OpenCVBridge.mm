...
-(bool)processFinger{ //2.1
    ...
    if (self.arrayID >= 99){
        sprintf(framesText, "100 Frames Filled");
    ...
    if ((avgPixelIntensity.val[2]<30 && avgPixelIntensity.val[1]<30) || (avgPixelIntensity.val[0]>200 && avgPixelIntensity.val[2]<30)) { //then read data into buffer, then return true
        if (self.arrayID < 100){ //when array full
            self.arrayID += 1;
